...
describe '投稿のテスト' do
  ...
  desctibe '投稿画面(todolists_new_path)のテスト' do
    before do
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        ...
        fill_in 'list[body]', with: Faker::Lorem.characters(bumber:20)
        click_button '投稿'
  ...
  describe '詳細画面のテスト' do
    ...
    context '表示のテスト' do
      it '削除リンクが存在しているか' do
        expect(page).to have_link "削除", href: destroy_todolist_path
      end
      it '編集リンクが存在しているか' do
        expect(page).to have_link "編集", href: edit_todolist_path
      end
    end
